...
class DetectorDBConnection:
    ...
    # def gather_detections_and_waveforms(
    #     self, session, continuous_data, is_p, pick_thresh, seconds_around_pick, on_event
    # ):
    #     # Setup
    #     data_id = self.daily_info.contdatainfo_id
    #     method_id = self.p_detection_method_id if is_p else self.s_detection_method_id
    #     phase = "P" if is_p else "S"

    #     # Get data info and detections
    #     cdi = session.get(DailyContDataInfo, data_id)
    #     samp_rate = cdi.samp_rate
    #     date = cdi.date
    #     prev_appended = cdi.prev_appended
    #     cdi_dict = {"id": data_id,
    #                 "samp_rate": samp_rate,
    #                 "prev_appended": prev_appended,
    #                 "date": date,
    #                 "proc_start": cdi.proc_start,
    #                 "dt": cdi.dt}
    #     samples_around_pick = int(seconds_around_pick * samp_rate)
    #     total_npts = len(continuous_data)
    #     dldets = services.get_dldetections(session, data_id, method_id, pick_thresh, phase)

    #     # Bundle detection and waveform detail info
    #     preprocessed = []
    #     for det in dldets:
    #         i1 = det.sample - samples_around_pick
    #         i2 = det.sample + samples_around_pick + 1
    #         wf_start_ind, wf_end_ind = 0, 2 * samples_around_pick + 1
    #         if i1 < 0:
    #             wf_start_ind = abs(i1)
    #             i1 = 0
    #         if i2 > total_npts:
    ...
    #         expected_array_length = 2 * samples_around_pick + 1

    #         preprocessed.append({
    #             "det": det,
    #             "pick_cont_data": pick_cont_data,
    #             "wf_start": wf_start,
    #             "wf_end": wf_end,
    #             "wf_start_ind": wf_start_ind,
    #             "wf_end_ind": wf_end_ind,
    #             "expected_array_length": expected_array_length,
    #             "samples": (i2 - i1),
    #         })

    #     return preprocessed, phase, cdi_dict
    
    # def insert_picks_and_waveform_metadata(self, session, preprocessed, is_p, phase, cdi_dict, auth, on_event):
    #     storage_dict = self._get_waveform_storages(session, is_p, {
    #         "expected_array_length": preprocessed[0]["expected_array_length"],
    #         "phase": phase,
    #         "on_event": on_event,
    #         "data_id": cdi_dict["id"],
    #         "use_pytables": True,
    ...
    #         det = record["det"]
    #         pick_waveform_details = record
    #         insert_new_pick = True

    #         # Handle previous-day detections
    #         if det.time.date() < cdi_dict["date"]:
    #             assert cdi_dict["prev_appended"]
    #             insert_new_pick = self._potentially_modify_pick_and_waveform(
    #                 session, storage_dict, det, pick_waveform_details, {
    #                     "expected_array_length": record["expected_array_length"],
    #                     "phase": phase,
    #                     "on_event": on_event,
    #                     "data_id": cdi_dict["id"],
    #                     "use_pytables": True,
    ...
    #                 session, self.station_id, self.seed_code, det.phase,
    #                 det.time, auth, detid=det.id
    #             )
    #             self._insert_new_waveforms(
    #                 session, storage_dict, pick, pick_waveform_details, {
    #                     "expected_array_length": record["expected_array_length"],
    #                     "phase": phase,
    #                     "on_event": on_event,
    #                     "data_id": cdi_dict["id"],
    ...
    # def save_picks_from_detections(self, pick_thresh, is_p, auth, continuous_data, seconds_around_pick, use_pytables=True, on_event=None):
    #     if not use_pytables:
    #         raise NotImplementedError("Only PyTables currently supported in refactored version")

    #     with self.Session() as session:
    #         with session.begin():
    #             preprocessed, phase, cdi_dict = self.gather_detections_and_waveforms(
    #                 session, continuous_data, is_p, pick_thresh, seconds_around_pick, on_event
    #             )
    ...
    #         try:
    #             with session.begin():
    #                 storage_dict = self.insert_picks_and_waveform_metadata(
    #                     session, preprocessed, is_p, phase, cdi_dict, auth, on_event
    #                 )
    #         except Exception:
    #             self.close_open_pytables()
    #             raise
    ...
    #         min_time=detection_time - timedelta(seconds=0.1),
    #         max_time=detection_time + timedelta(seconds=0.1),
    #     )
    ...
    #     pick.ptime = detection_time
    #     pick.detid = detection_id
