...
class ApplyDetector:
    ...
    def __init_1c(self):
        ...
        if self.use_openvino:
            self.p_detector.compile_openvino_model(
                self.window_length, self.batchsize, self.use_async
            )
    ...
    def __init_3c(self):
        ...
        if self.use_openvino:
            self.s_detector.compile_openvino_model(
                self.window_length, self.batchsize, self.use_async
            )
    ...
    def __apply_to_one_phase(
        ...
        """
        starttime = time.time()
        data, start_pad_npts, end_pad_npts = self.dataloader.format_continuous_for_unet(
            self.window_length, self.sliding_interval, proc_func, normalize=True
        )
        logger.debug(
        ...
        if debug_N_examples > 0:
            ...
            self.dataloader.metadata["npts"] = int(
                debug_N_examples * self.center_window * 2
            )
            # Ensure don't trim the post probs
            start_pad_npts, end_pad_npts = self.window_edge_npts, self.window_edge_npts

        cont_post_probs = detector.get_continuous_post_probs(
            data,
            self.center_window,
    ...
    def validate_date_range(stat_startdate, stat_enddate, run_startdate, n_days):
        ...
        if run_startdate < stat_startdate:
            ...
            run_startdate = datetime.datetime(
                year=stat_startdate.year,
...
class PhaseDetector:
    ...
    def __init_model(self, num_channels, model_to_load):
        ...
        self.unet = UNetModel(
            num_channels=num_channels, num_classes=1, apply_last_sigmoid=True
        ).to(self.torch_device)
        logger.info(
            f"Initialized {num_channels} comp {self.phase_type} unet with {self.get_n_params()} params..."
        )
    ...
    def compile_openvino_model(self, window_length, batchsize, use_async):
        ...
        ov_model = ov.convert_model(
            self.unet,
            input=([batchsize, self.num_channels, window_length], ov.Type.f32),
        )
        ...
        ov_compiled_model = core.compile_model(
            ...
            config={
                ov.properties.inference_num_threads(): n_threads,
    ...
    def apply_async_openvino(self, input, batchsize=2, center_window=None):
        ...
        while sind < n_examples:
            ...
            for i in range(sind, eind, batchsize):
                shared_tensor = ov.Tensor(
                    input[i : i + batchsize, :, :], shared_memory=True
                )
    ...
    def apply_sync(self, input, batchsize=256, center_window=None):
        ...
        while batch_start < n_examples:
            ...
            if not self.openvino_compiled:
                post_probs[batch_start:batch_end, :] = self.apply_model_to_batch(
                    batch, center_window=center_window
                )
            else:
                post_probs[batch_start:batch_end, :] = (
                    self.apply_sync_openvino_model_to_batch(
                        batch, center_window=center_window
                    )
                )
    ...
    def get_n_params(self):
        ...
        if self.openvino_compiled:
            logger.warning(
                "Can only return number of params for Torch model, not OpenVino"
            )
            return None

        return sum(p.numel() for p in self.unet.parameters() if p.requires_grad)

    def get_continuous_post_probs(
        self,
        input,
        center_window,
        window_edge_npts,
        ...
        if self.use_openvino_async:
            unet_output = self.apply_async_openvino(
                input, center_window=center_window, batchsize=batchsize
            )
        else:
            unet_output = self.apply_sync(
                input, center_window=center_window, batchsize=batchsize
            )
        cont_post_probs = self.flatten_model_output(unet_output)
        cont_post_probs = self.trim_post_probs(
    ...
    def save_post_probs_miniseed(outfile, post_probs, stats):
        ...
        """
        assert (
            post_probs.shape[0] == stats["npts"]
        ), "posterior probability is the wrong shape"
    ...
    def trim_post_probs(post_probs, start_pad, end_pad, edge_n_samples):
        ...
        """
        assert (
            len(post_probs.shape) == 1
        ), "Post probs need to be flattened before trimming"
...
class DataLoader:
    ...
    def load_3c_data(
        self, fileE, fileN, fileZ, min_signal_percent=1, expected_file_duration_s=86400
    ):
        ...
        assert np.isin(
            st_N[0].stats.channel, ["EHN", "EH2", "BHN", "BH2", "HHN", "HH2"]
        ), "N file is incorrect"
        assert np.isin(
            st_Z[0].stats.channel, ["EHZ", "BHZ", "HHZ"]
        ), "Z file is incorrect"
        ...
        starttimes = [
            ...
            st_N[0].stats.starttime,
            st_Z[0].stats.starttime,
        ]
    ...
    def format_continuous_for_unet(
        ...
        total_npts, start_pad_npts, end_pad_npts = self.get_padding(
            npts, unet_window_length, unet_sliding_interval, pad_start
        )
        n_windows = self.get_n_windows(
            total_npts, unet_window_length, unet_sliding_interval
        )
        window_start_indices = self.get_sliding_window_start_inds(
            total_npts, unet_window_length, unet_sliding_interval
        )
        ...
        data = self.add_padding(
            np.copy(self.continuous_data), start_pad_npts, end_pad_npts
        )
    ...
    def prepend_previous_data(self):
        ...
        if self.previous_continuous_data is not None:
            ...
            if (current_starttime - self.previous_endtime) < self.metadata[
                "dt"
            ] * 1.5 and (current_starttime - self.previous_endtime) > 0:
                self.continuous_data = np.concatenate(
                    [self.previous_continuous_data, self.continuous_data]
                )
                self.metadata["starttime"] = (
                    self.metadata["starttime"] - self.store_N_seconds
                )
                self.metadata["starttime_epoch"] = self.metadata["starttime"] - UTC(
                    "19700101"
                )
                ...
                logger.warning(
                    "Cannot concatenate previous days data, data is not continuous"
                )
    ...
    def store_metadata(self, stats, three_channels=True):
        ...
        meta_data["location"] = stats["location"]
        chan = stats["channel"]
        if three_channels:
            chan = f"{chan[0:2]}?"
    ...
    def load_channel_data(
        ...
        if expected_file_duration_s != 24 * 60 * 60:
            raise NotImplementedError(
                "DataLoader only works for day long files at the moment..."
            )
        ...
        if (total_npts / max_npts) * 100 < min_signal_percent:
            logger.warning(
                f"{os.path.basename(file)} does not have enough data, skipping"
            )
        ...
        if len(st.get_gaps()) > 0:
            ...
            try:
                ...
                logger.info(
                    f"Caught obspy Incompatible Traces warning: {e}. Fixing the sampling rates..."
                )
    ...
    def write_data_info(self, outpath):
        ...
        with open(outpath, "w") as fp:
            json.dump(gap_dict, fp, sort_keys=True, indent=4, ensure_ascii=False)
    ...
    def get_padding(npts, unet_window_length, unet_sliding_interval, pad_start=True):
        ...
        end_pad_npts = (
            unet_sliding_interval
            - (npts_padded - unet_window_length) % unet_sliding_interval
        )
    ...
    @staticmethod
    def get_sliding_window_start_inds(
        npts_padded, unet_window_length, unet_sliding_interval
    ):
    ...
    @staticmethod
    def format_edge_gaps(
        st, desired_start, desired_end, entire_file=False, interpolated=True
    ):
        ...
        if entire_file:
            ...
            return [
                ...
                desired_end,
                max_duration_s,
                int(max_duration_s * sampling_rate),
            ]
        ...
        if end_delta > dt:
            end_gap = [
                ...
                st[end_gap_st_ind].stats.location,
                st[end_gap_st_ind].stats.channel,
                endtime,
                desired_end,
                end_delta,
                int(end_delta * sampling_rate),
            ]
