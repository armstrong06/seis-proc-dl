...
class TestMultiSWAGPickerDB:
    ...
    @pytest.fixture
    def p_ex_paths(self):
        selected_model_path = "/uufs/chpc.utah.edu/common/home/koper-group3/alysha/selected_models"
    ...
    def init_p_picker(self, p_ex_paths):
        ...
        sp = apply_swag_pickers.MultiSWAGPickerDB(is_p_picker=True, 
    ...
    def init_s_picker(self, s_ex_paths):
        ...
        sp = apply_swag_pickers.MultiSWAGPickerDB(is_p_picker=False, 
                                                  swag_model_dir=model_path,
                                                  cal_model_file=cal_file)
        return sp
    ...
    def test_process_3c_S(self, init_s_picker):
        ...
        assert np.allclose(proc_data, ref_proc_data, 1.e-2)
    ...
    def test_process_1c_P(self, init_p_picker):
        ...
        assert np.allclose(proc_data, ref_proc_data[:, 1:], 1.e-2)
        assert np.max(np.abs(proc_data)) > 1, "data is normalized"
    ...
    def test_get_multiple_cis(self, init_p_picker):
        ...
        assert cis_summary[68]['arrivalTimeShiftLowerBound'][0] < -1 and cis_summary[68]['arrivalTimeShiftLowerBound'][0] > -2
        assert cis_summary[68]['arrivalTimeShiftUpperBound'][0] > 1 and cis_summary[68]['arrivalTimeShiftUpperBound'][0] < 2
    ...
    def test_trim_dists(self, init_p_picker):
        ...
        for i in range(2):
            ...
            assert abs(trim_results["trim_median"][i]  - org_medians[i]) < 0.05, "medians should be close"
    ...
    def test_get_summary_stats(self, init_p_picker):
        ...
        for i in range(2):
            ...
            assert abs(summary_dict["trim_median"][i]  - org_medians[i]) < 0.05, "medians should be close"
